...
@app.command()
def run(
    mode: str = typer.Option("nano", help="Run mode: nano (~30s), quick (~5min), standard (~1hr), full (~4hr)"),
    ...
    async def run_evaluation():
        ...
        with Progress(
            ...
            async with ForgeRunner(model_config, concurrency) as runner:
                for cat_name in category_list:
                    progress.update(task, description=f"Running {cat_name}...")
                    ...
                    result = await runner.run_category(
                        category,
                        run_config.difficulty_distribution,
                    )
        ...
        # Save results
        save_results(
            output, run_config, model_config, results,
            forge_score, interp, extra, global_cliff, elapsed,
        )
...
def save_results(output_path, run_config, model_config, results,
                 forge_score, interp, extra, cliff_index, elapsed):
        ...
        "categories": [
            {
                ...
                "accuracy_by_difficulty": {
                    str(k): v for k, v in r.accuracy_by_difficulty.items()
                },
            }
            for r in results
        ],
