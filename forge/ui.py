...
def create_ui():
    ...
    with gr.Blocks(
            ...
            with gr.TabItem("Single Model"):
                with gr.Row():
                    with gr.Column(scale=1):
                        ...
                        connection_status = gr.Markdown("*Click 'Test Connection' to verify API access*")
                ...
                with gr.Tabs():
                    ...
                    with gr.TabItem("Comparison Table"):
                        comparison_table = gr.Dataframe(
                            headers=[
                                "Rank", "Model", "FORGE Score", "Interpolation", 
                                "Extrapolation", "Cliff Index", "Cost ($)", "Runtime"
                            ],
                            datatype=["number", "str", "number", "number", "number", "str", "number", "number"],
                            interactive=False,
                        )
        ...
        test_btn.click(
            fn=lambda api_base, api_key, model: asyncio.run(test_connection(api_base, api_key, model)),
        ...
        def add_model(name, api_base, api_key, input_cost, output_cost, current_models):
            ...
            # Format for display
            display_data = [
                [m["name"], m["api_base"], m["input_cost"], m["output_cost"]]
                for m in updated
            ]
        ...
        add_model_btn.click(
            fn=add_model,
            inputs=[
                multi_model_name, multi_api_base, multi_api_key,
                multi_input_cost, multi_output_cost, models_state,
            ],
        ...
        def run_single_evaluation(model, api_base, api_key, input_cost, output_cost,
                                 mode, seed, categories_str, concurrency_val):
            return asyncio.run(_evaluate_single_model(
                model, api_base, api_key, input_cost, output_cost,
                mode, seed, categories_str, concurrency_val
            ))
        ...
        run_btn.click(
            ...
            inputs=[
                model_name, api_base, api_key, input_cost, output_cost,
                run_mode, seed, categories_input, concurrency,
            ],
        ...
        def run_multi_evaluation(models, mode, seed, categories_str, concurrency_val):
            ...
            return asyncio.run(_evaluate_multiple_models(
                models, mode, seed, categories_str, concurrency_val
            ))
        
        run_all_btn.click(
            fn=run_multi_evaluation,
            inputs=[
                models_state, multi_run_mode, multi_seed,
                multi_categories, multi_concurrency,
            ],
            outputs=[
                leaderboard_display, comparison_table,
                export_json, per_category_display,
            ],
        )
...
async def _evaluate_single_model(model, api_base, api_key, input_cost, output_cost,
                                 mode, seed, categories_str, concurrency_val):
    ...
    try:
        async with ForgeRunner(model_config, int(concurrency_val)) as runner:
            ...
            for cat_name in category_list:
                ...
                result = await runner.run_category(category, run_config.difficulty_distribution)
...
async def _evaluate_multiple_models(models, mode, seed, categories_str, concurrency_val):
    """Run evaluation for multiple models and generate leaderboard."""
    ...
    for i, model_info in enumerate(models):
        ...
        gr.Info(f"Evaluating model {i+1}/{len(models)}: {model_name}")
...
def _get_run_configs(category_list):
    ...
    nano_categories = ["arithmetic_chain", "mod_exp", "game_nim", "info_entropy", "combinatorics_stars"]
...
def _format_leaderboard(all_results):
    ...
    md += f"- **Best extrapolation**: {best_extra[0]} ({best_extra[1]['extrapolation_score']:.4f})\n"
